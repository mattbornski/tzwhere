    ...
    if (false) {
      ...
      for (var lngDeg in timezoneLongitudeShortcuts) {
        for (var latDeg in timezoneLatitudeShortcuts) {
          ...
          var applicableTimezones = lngSet.intersection(latSet);
          avgTzPerShortcut += applicableTimezones.size();
        }
      }
...
var dateIn = function () {
  ...
  if (arguments.length === 0) {
    ...
    // If no date constructor given, assume "now".
    args = [new Date(), arguments[0]];
  } else {
    args = arguments;
  }
  return time.Date.apply(new time.Date(), args);
};
...
var tzOffsetAt = function () {
  var tzname = tzNameAt(arguments[0], arguments[1]);
  if (tzname) {
    var comparisonDate = Date.apply(new Date(), Array.prototype.slice.call(arguments, 2));
