...
var constructionLock = path.join(__dirname, 'shortcuts.lock');
var cache;
...
var load = function () {
	...
	if (fs.existsSync(constructedShortcutFilePath)) {
		...
		events.emit('loaded', { from: 'calc', time: Date.now() - now });
	} else if (fs.existsSync(constructionLock)) {
		events.emit('lock', { state: 'watching', time: Date.now() - now });
		fs.watchFile(constructionLock, { interval: 300 }, function (curr) {
			...
			events.emit('lock', { state: 'unlock', time: Date.now() - now });
