...
var init = function(tzWorldFile, options){
  events.emit('loading');
  options = options || {};

  constructedShortcutFilePath = path.join(options.tmpDir || __dirname, 'shortcuts.json');
  constructionLock = path.join(options.tmpDir || __dirname, 'shortcuts.lock');
