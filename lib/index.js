...
var init = function(tzWorldFile, callback){
  ...
  if(currentTzWorld !== tzWorldFile) {
    ...
    if (callback) {
      ...
      fs.readFile(tzWorldFile, 'utf-8', function(error, data) {
        if (error) {
          callback(error);
        } else {
          try {
            constructShortcuts(JSON.parse(data));
            callback(null);
          } catch(e){
            callback(e);
          }
      ...
      constructShortcuts(featureCollection);
    }
    return true;
  } else {
    return false
...
var constructShortcuts = function (featureCollection) {
  // Construct once
